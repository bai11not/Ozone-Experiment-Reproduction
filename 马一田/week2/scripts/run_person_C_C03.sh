#!/bin/bash
# ============================================================
# Week 2: Person C 批量消融实验 (续跑 C03-C08)
# seed=52, full + no_diff
# 用法: bash run_person_C_C03.sh
# ============================================================
set -euo pipefail
...
N_NODE=95; M=15; HIDDEN_SIZE=64; BATCH_SIZE=16; LR=7e-4; EPOCHS=120; PATIENCE=15
DIFF_STEPS=50; INFERENCE_STEPS=50; NUM_SAMPLES=3; USE_ADAPTIVE_ADJ=1; PE_SOURCE="train"
AMP=1; PE_WINDOW_STEP=168; EVAL_INFERENCE_STEPS=50; EVAL_NUM_SAMPLES=3
SAVE_PREDICTIONS=1; LOG_INTERVAL=50; USE_MET_CACHE=1; MAX_WINDOWS=0

TOTAL=6; CURRENT=0

run_exp() {
  local SEED=$1 SEQ_LEN=$2 PRE_LEN=$3 USE_DIFFUSION=$4 USE_PE_GRAPH=$5 USE_PE_FILM=$6 EXP_LABEL=$7
  CURRENT=$((CURRENT + 1))
...

  {
    ...
    echo "=========================================="
    echo "Label: ${EXP_LABEL} | Seed: ${SEED} | Seq: ${SEQ_LEN} | Pre: ${PRE_LEN}"
    echo "Diff: ${USE_DIFFUSION} | PE Graph: ${USE_PE_GRAPH} | PE FiLM: ${USE_PE_FILM}"
    echo "Output: ${OUTPUT_DIR} | Start: $(date '+%Y-%m-%d %H:%M:%S')"
    echo "=========================================="
    ...
    EXIT_CODE=$?
    echo ""
    echo "--- [${CURRENT}/${TOTAL}] ${EXP_LABEL} finished at $(date '+%Y-%m-%d %H:%M:%S') | Exit: ${EXIT_CODE} ---"
...
echo "============================================="
echo "Week 2: Person C - C03-C08 (C01,C02 done)"
echo "Log: ${LOG_FILE} | Started: $(date '+%Y-%m-%d %H:%M:%S')"
echo "============================================="
...
echo ""
echo "============================================="
echo "Person C: C03-C08 all done! Finished: $(date '+%Y-%m-%d %H:%M:%S')"
echo "============================================="
