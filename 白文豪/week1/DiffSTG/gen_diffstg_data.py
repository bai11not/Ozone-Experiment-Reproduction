...
PROJECT = r"d:\时空数据\臭氧预测资料"
MATRIX = os.path.join(PROJECT, "matrix_N95")
STATION_XLSX = os.path.join(PROJECT, "xlsx_N95", "station_loc1.xlsx")
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "dataset", "AIR_N95")

print("=" * 50)
print("Step 1: 生成 flow.npy")
data = np.load(os.path.join(MATRIX, "data_combined_m15.npy"))
print(f"  data_combined_m15: {data.shape}")

o3 = data[:, :, 0:1].astype(np.float32)
...
lng = df.iloc[:, 3].values.astype(np.float64)
lat = df.iloc[:, 4].values.astype(np.float64)
print(f"  站点数: {len(lng)}")
